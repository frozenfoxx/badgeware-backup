...
ENV HOME=/root \
  ...
  BUILD_DEPS="cmake git libusb-1.0-0-dev pkg-config" \
...
# Install packages and build picotool from source
RUN apt-get update && \
  apt-get upgrade -y && \
  ...
  cmake -S /tmp/picotool -B /tmp/picotool/build -DPICOTOOL_NO_LIBUSB_FALLBACK=OFF && \
